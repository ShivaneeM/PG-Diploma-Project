...
const EditProfileScreen = (props) => {
  ...
  let { id, firstName, lastName, email, password, phone, role } =
  ...
  const onEditUser = () => {
    if (firstName_o.length < 1) setFNameError("First Name is mandatory");
    else setFNameError("");

    if (password_o.length < 1) setPasswordError("Password is required");
    else setPasswordError("");

    if (firstName_o.length > 1 && password_o.length > 1)
      dispatch(
        editUser(id, firstName_o, lastName_o, email, password_o, role, phone_o)
      );
  };
  ...
  useEffect(() => {
    if (response && response.status === "success") {
      ...
      dispatch(signout());
      toast.success("Logging out for saving changes", {
        autoClose: 2000,
